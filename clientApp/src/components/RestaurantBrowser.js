...
export default function RestaurantBrowser(props) {
  ...
  // Funktiolla lisätään headeriin kaupungin valinnan postonappi sekä napit ravintolatyylifintteröintiin
  const manageHeaderContent = (props) => {
    ...
    function manageRestaurantStyleButtons(){
      if( restauranStyle.length === 1){
        return <button className="styleButton" type="button" onClick={ ()=> getData().then(listStyles) }><span>{ restauranStyle[0] } <FontAwesomeIcon style={{ color: 'crimson' }} icon={ faXmark }/></span></button>
      }
      else{
        return restauranStyle.map((item, index) => {
          return <button className="styleButton" type="button" key={index} onClick={ ()=> setRestauranStyle([item]) }><span>{ item }</span></button>
        });
      }
    }
    ...
    return(
      <div className="flex">
        <button className="cityButton" type="button" onClick={ ()=> testiFunktio("") }><span>{ selectedCity } <FontAwesomeIcon style={{ color: 'crimson' }} icon={ faXmark }/></span></button>
  ...
  return (
    <div>
      <Header onSearchButtonClick={ searchHandler } testi={ testiFunktio } addContentToHeader={ manageHeaderContent }/>
